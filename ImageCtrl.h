...
#define CMD_SAVE_VIEW_IMAGE (99999)
#define DEAFALUT_AVOID_VAL (-100)
...
public:
	...
	void EnableGrid(BOOL bEnableGrid) { m_bEnbleGrid = bEnableGrid; }                                            // Loads an image from BYTE array.
	void SetFindMarkPoint(CPoint pnt, long Diamater) { m_pntFindMarkingPnts = pnt; m_lDiameterOfFindMark = Diamater; }
	void EnableShapeDrawing(BOOL Shape);
	void SetResult(double ShiftX, double ShiftY) { m_dShiftXinmm = ShiftX; m_dShiftYinmm = ShiftY; }
	void SetImage(Bitmap* Imgptr);
	BOOL load(BYTE * pData, size_t nSize);
	void ClearViewData();
	void EnableZoomFit(BOOL zoomFit) { m_bEnableZoomFit = zoomFit; }
private:
	...
	CCriticalSection          m_CtriUiDraw;
	BOOL                      m_bEnbleGrid;
	CPoint      			  m_pntFindMarkingPnts;
	long                      m_lDiameterOfFindMark;
	double                    m_dCameraResulotionFactor;
	BOOL                      m_bEnbleShapeDrawing;
	double                    m_dShiftXinmm= DEAFALUT_AVOID_VAL;
	double                    m_dShiftYinmm= DEAFALUT_AVOID_VAL;
	BOOL                      m_bEnableZoomFit = FALSE;
protected:
	BOOL load(Bitmap * imgPtr);
	virtual void PreSubclassWindow();
	afx_msg void OnLButtonDown(UINT nFlags, CPoint point);
	afx_msg void OnLButtonUp(UINT nFlags, CPoint point);
	afx_msg void OnMouseMove(UINT nFlags, CPoint point);
	afx_msg BOOL OnMouseWheel(UINT nFlags, short zDelta, CPoint pt);
	void DrawGridOnView(CDC * dcMem, CRect rct);
	void DrawPlusMarkOnFindPoint(CDC * dcMem, double dXratio, double dYratio);
	void DrawFindShape(CDC * dcMem, double dXratio, double dYratio);
	void DrawResultTextOnView(CDC * dc);
	DECLARE_MESSAGE_MAP()
public:
	afx_msg void OnPaint();
	afx_msg void OnLButtonDblClk(UINT nFlags, CPoint point);
	void EnableDiasblePan(bool bPan);
